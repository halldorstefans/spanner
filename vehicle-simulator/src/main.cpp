...
namespace {
    ...
    void print_usage(const char* prog) {
        std::cout << "Usage: " << prog << " <broker_address> [options]\n"
                  << "\nArguments:\n"
                  << "  broker_address     MQTT broker address (required, positional argument)\n"
    ...
    Config parse_args(int argc, char* argv[]) {
        ...
        for (int i = 2; i < argc; ++i) {
            if (std::strcmp(argv[i], "-r") == 0 || std::strcmp(argv[i], "--rate") == 0) {
                if (i + 1 < argc) {
                    config.rate_hz = std::stoi(argv[++i]);
                }
            } else if (std::strcmp(argv[i], "-v") == 0 || std::strcmp(argv[i], "--vin") == 0) {
                if (i + 1 < argc) {
                    config.vin = argv[++i];
                }
            }
